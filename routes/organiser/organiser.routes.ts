...
organiser.post('/auth/finish', async c => {
  ...
  if(connection){
    ...
    const token = await new SignJWT({ id: organiser_id, sid: session.id.id.toString(), role: 'attendee' }).setProtectedHeader({ alg: 'HS256' }).setIssuedAt().setExpirationTime(jwt_expiration).sign(encodedSecret)
